...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        // 如果是第一次, 则创建空的服务, 即namespace下的group下的service, 也就是Instance的容器
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
        ...
        // 从注册表中拿到service
        Service service = getService(namespaceId, serviceName);
        // 由于一个服务存在多个实例, 那么在多线程(实例)同时操作一个资源时, 加上锁, 防止脏写问题
        // 以service为锁对象, 同一个服务的多个实例只能串行来完成注册
        synchronized (service) {
            // 拷贝注册表老的实例列表, 加上参数传递进来的新注册的实例, 得到最新的实例列表 COPY On Write
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips);

            // 将新的实例列表封装到Instances对象中
            Instances instances = new Instances();
            instances.setInstanceList(instanceList);

            // 更新注册表(1. 更新本地注册表 2. 数据同步给nacos集群中的其他节点), 最耗时, 但是内部采用阻塞队列和线程池异步执行
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips)
            throws NacosException {
        // 从DataStore中获取实例列表, 可以理解为nacos集群中同步获取到的实例列表
        ...
        if (datum != null && null != datum.value) {
            // 如果集群同步列表中有数据, 则将本地注册列表与datum中获取到的列表 做对比
        ...
        for (Instance instance : ips) {
            ...
            if (UtilsAndCommons.UPDATE_INSTANCE_ACTION_REMOVE.equals(action)) {
                ...
                // 尝试从旧的实例列表中获取新的实例列表中的实例
                Instance oldInstance = instanceMap.get(instance.getDatumKey());
                if (oldInstance != null) {
                    // 如果新的实例列表中存在与旧的实例ip和端口相同的, 那么将旧的实例的instanceId赋值给新的实例(实例重启)
                    instance.setInstanceId(oldInstance.getInstanceId());
                } else {
                    // 如果不存在, 则说明是一个全新实例, 生成一个instanceId赋值给它
    ...
    public Service getService(String namespaceId, String serviceName) {
        ...
        // 如果serviceMap已经存在, 则尝试从serviceMap中获取对应的service
