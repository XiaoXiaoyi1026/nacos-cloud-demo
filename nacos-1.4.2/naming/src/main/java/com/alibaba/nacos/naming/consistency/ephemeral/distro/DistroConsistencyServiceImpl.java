...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService, DistroDataProcessor {
    ...
    public void put(String key, Record value) throws NacosException {
        // 更新本地注册表(异步, 将更新任务扔给阻塞队列, 由线程池异步获取任务并执行)
        onPut(key, value);
        // 同步数据给nacos集群中的其他节点(同样的异步执行)
    ...
    public void onPut(String key, Record value) {
        ...
        // 把serviceId和操作类型存入notifier的task列表中
    ...
    public class Notifier implements Runnable {
        ...
        public void addTask(String datumKey, DataOperation action) {
            ...
            // 把serviceId和CHANGE事件放入BlockQueue阻塞队列中, 是nacos做的异步性能优化, 并不马上更新本地实例列表
        ...
        public void run() {
            ...
            for (; ; ) {
                try {
                    // 从阻塞队列中获取任务, 由于tasks是阻塞队列, 其中存在对象时才会返回, 不存在时会wait(), 存在等待唤醒机制
        ...
        private void handle(Pair<String, DataOperation> pair) {
            try {
                ...
                for (RecordListener listener : listeners.get(datumKey)) {
                    ...
                    try {
                        if (action == DataOperation.CHANGE) {
                            // 此处的listener就是service对象本身, 当service服务发生变化时, 触发onChange事件, 以处理变更
